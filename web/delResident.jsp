...
<div id="login">
    <form action="/SystemControl?method=resident_del" method="post">
        <c:forEach var="row" items="${result.rows}">
            <div class="text">你确定要删除以下对象的信息吗？</div>
            <div class="text">序号:<input class="t1" type="text" name="resident_index" readonly
                                          value="${row.resident_index}"></div>
            <div class="text">姓名:<input class="t1" type="text" name="resident_name" readonly
                                          value="${row.resident_name}"></div>
        </c:forEach>
        <div id="register">
            <button type="submit" style="background-color: red">删除</button>
            <a href="/SystemControl?method=resident_list" style="text-decoration: none">
