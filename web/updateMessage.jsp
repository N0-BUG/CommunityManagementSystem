...
<div id="login">
    <form action="/MessageControl?method=update" method="post">
        <div id="head">在这里修改留言信息</div>
        <c:forEach var="row" items="${result.rows}"> <%--考虑到对序号不做更新，设置为只读readonly--%>
            <div class="text">&nbsp&nbsp&nbsp&nbsp序号:<input class="t1" type="text" name="message_index" readonly
                                                              value="${row.message_index}"></div>
            <div class="text">&nbsp&nbsp留言人:<input class="t1" type="text" name="message_publisher"
                                                      value="${row.message_publisher}"></div>
            <div class="text">留言信息:<input class="t1" type="text" name="message_info" value="${row.message_info}">
            </div>
            <div class="text">留言时间:<input class="t1" type="text" name="message_date" value="${row.message_date}">
            </div>
        </c:forEach>
        <div id="register">
