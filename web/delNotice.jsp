...
<div id="login">
    <form action="/NoticeControl?method=del" method="post">
        <c:forEach var="row" items="${result.rows}">
            <div class="text">你确定要删除以下对象的信息吗？</div>
            <div class="text">序号:<input class="t1" type="text" name="notice_index" readonly
                                          value="${row.notice_index}"></div>
            <div class="text">发布人:<input class="t1" type="text" name="notice_publisher" readonly
                                            value="${row.notice_publisher}"></div>
        </c:forEach>
        <div id="register">
            <button type="submit" style="background-color: red">删除</button>
            <a href="/NoticeControl?method=list" style="text-decoration: none">
