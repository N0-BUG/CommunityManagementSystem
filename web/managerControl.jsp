...
<div id="right">
    <form method="post" action="/ManagerControl?method=search">
        <div id="right_text1">查询字段:
            <select name="key" style="width: 100px;height: 30px">
                <option value="manager_name">管理员姓名</option>
                <option value="manager_post">管理员职位</option>
            </select></div>
    ...
    <div id="table">
        <table>
            ...
            <c:forEach items="${list}" var="Manager">
                <tr>
                    <td>${Manager.manager_index}</td>
                    <td>${Manager.manager_name}</td>
                    <td>${Manager.manager_phone}</td>
                    <td>${Manager.manager_gender}</td>
                    <td>${Manager.manager_post}</td>
                </tr>
            </c:forEach>
            </tbody>
        </table>
